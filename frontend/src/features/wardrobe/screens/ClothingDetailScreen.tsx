...
export function ClothingDetailScreen() {
  ...
  // Populate form when item loads or edit mode opens
  useEffect(() => {
    if (item && isEditMode) {
      setName(item.name);
          ...
          <>
            ...
            <View style={styles.infoGrid}>
              ...
              {item.seasons && item.seasons.length > 0 && (
                <InfoRow label="Mevsimler" value={item.seasons.map((s) => CLOTHING_SEASON_LABELS[s] ?? s).join(', ')} />
              )}
              {item.styles && item.styles.length > 0 && (
                <InfoRow label="Stiller" value={item.styles.map((st) => CLOTHING_STYLE_LABELS[st] ?? st).join(', ')} />
              )}
...
const styles = StyleSheet.create({
  ...
  itemName: { flex: 1, fontSize: 22, fontWeight: '700', color: '#1E1B4B' },
  cleanBadge: {
  ...
  errorMsg: { color: '#EF4444', fontSize: 14, marginBottom: 10 },
